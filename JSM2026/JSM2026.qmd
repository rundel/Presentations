...
Having adopted GitHub, you still have to decide how to manage things there.
...
One org per course per semester. Students added  *members* (to use teams). The default repository permission is set to `none`, so students only see their assigned repos.
...
Use base R and ghclass to manage invites:
...
::: {.medium}
* `hw1` - the template, holding the student scaffold and the CI config

* `hw1-key` - the answer key

* `hw1_lab01_team01`, ... - one repo per team/individual, mirrored from the template
:::
...
Per assignment we build a roster that tracks student details and team assignment,
...
::: {.aside}
Teams are just a column in the roster - these can also be manual created or edited
:::
...
* We include workflows in student repos (`.github/workflows/check_assignment.yml`) as part of the template

* The checks are plain R functions from [`checklist`](https://github.com/rundel/checklist) - required files present, document renders, etc.

* A prebuilt container image keeps runs fast and the environment consistent
...
Numeric scores go to the gradebook, written feedback via GitHub issues
...
An interactive (Shiny) grading interface for Quarto and R Markdown assignments, built on top of [`q2r`](https://rundel.github.io/q2r) that works with `org_grade_assignment()`.
...
A grading project (folder) is initialized with `init_project()`.

* `template()` is used to build an assignment template - document subsections and validation rules

* `mark()` is the grading interface that lets you build a rubric and apply it across student submissions

* `export_marks()` collects results from the database and populates `scores.csv` and `comments/`


## `markermd` and AI

Templates, rubrics, and marks round-trip from database as schema-validated YAML - deliberately simple enough for an LLM to author.

`markermd::init_project()` installs three Claude Code skills in the grading folder.

::: {.medium}
* `markermd-scaffold-template` - questions and structural rules from the key

* `markermd-scaffold-rubric` - rubric items, informed by sampling submissions

* `markermd-apply-rubric` - a first pass across every repository, pre-toggled for review
:::

Control over whether the LLM sees student work - some support for `ghclass::local_repo_anonymize()` for best effort anonymization of grading project.

## Some advice

* Each org gets 2,000 GitHub Actions minutes / month free, 3,000 on Team

  * easy to exhaust for a medium-sized course - upgrade, or self-host runners
...
* Beware of secondary rate limits, e.g. bulk invitations and bulk issue creation
  ...
  * a free org caps out around 50 invites/day

  * the fix is the free Team upgrade via [GitHub Education](https://education.github.com)
