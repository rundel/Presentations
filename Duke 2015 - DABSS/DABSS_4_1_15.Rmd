...
Lets say we want to fly from the Western most point in the US to the Eastern most point.


It turns out this depends on our definition of east and west - if we use latitude and longitude
then:
...
How do we define the distance between A and B or A and C?
...
row.names(nc) = as.character(nc$COUNTY)
...
str(adj)
nc$COUNTY[adj["Durham County",]]
```

##

```{r fig.align="center"}
image(adj,col=grey(c(1,0)),axes=FALSE)
box()
```
